...
public class TestRandomValue  {
	...
	public String getFailedFunctions() {
		if ( null == failedFunctions || 0 == failedFunctions.size()) return ("None");
		else return listFailedFunctions();
	}

	private String listFailedFunctions() {
		StringBuilder sb = new StringBuilder();
		boolean isFirst = true;
		for (String failedFunc : failedFunctions) {
			if ( isFirst ) isFirst = false;
			else sb.append(',');
			sb.append(failedFunc);
		}
		return sb.toString();
	}
	...
	protected boolean isRunMethod(Method runMethod) {
	...
	public String toString() {
		...
		if ( null == failedFunctions || 0 == failedFunctions.size())
			sb.append("None");
		else sb.append(listFailedFunctions() );
		return sb.toString();
	}
